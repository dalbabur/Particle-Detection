...
all_y = xlsread('analyzed.xlsx','all_y');
...
numT = zeros(1,size(all_y,2)); numB = zeros(size(numT));
for i = 1:size(all_y,2)
    dummy = histcounts(all_y(:,i));
    numT(i) = max(dummy);
    numB(i) = min(dummy(dummy>0));
end
...
for j = [9,10,11,13,14,12]
    ...
    dummyB = [];
    for i = 1:500
        dummyT(:,i) = randperm(info{j,10}*round(n_frames/(info{j,10}/info{j,15})),numT(j-8));
        dummyB(:,i) = randperm(info{j,10}*round(n_frames/(info{j,10}/info{j,15})),numB(j-8));
    end
    yT = ones(size(dummyT))*max(all_y(:,j-8));
    yB = ones(size(dummyT))*min(all_y(:,j-8));
    [randlocs,idx] = sort([dummyT; dummyB]);
    ylocs = [yT;yB]; ylocs = ylocs(idx);
    clear yT yB dummyT dummyB
    randdist = zeros(500,length(randlocs)-1);
    d_types = zeros(500,length(randlocs)-1);
    for i = 1:(length(randlocs)-1)
        randdist(:,i) = arrayfun(@(x1,y1,x2,y2) norm([x1,y1]-[x2,y2]),randlocs(i+1,:),ylocs(i+1,:),randlocs(i,:), ylocs(i,:));
        d_types(:,i) = ylocs(i+1,:) == ylocs(i,:);
    end
    d_types = logical(d_types);
