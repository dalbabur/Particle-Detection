function hAxes = dscatter_SJ(X,Y, varargin)
...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% % Modified by SJ March 2011
%ColorScheme, Red, Green, Blue, Purple, Dirty
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%         
% 
...
if nargin > 2
    ...
    okargs = {'colorscheme','smoothing','bins','plottype','logy','marker','msize','filled'};
    for j=1:2:nargin-2
        pname = varargin{j};
        pval = varargin{j+1};
...
elseif length(k)>1
    ...
    switch(k)
        ...
        case 5 %'scatter'
            %c1=[1 1 1];%Brightest color in the plot is always white
            switch ColorScheme
                case {'Red'}
                    c1=rgb('Pink');%brightest color
                    c2=rgb('Crimson');%Middle Color
                    c3=rgb('DarkRed');%Darkest color
                    c=makeColorMap(c3, c2, c1);
                case {'BrightGreen'}
                    c1=rgb('PaleGreen');%brightest color
                    c2=rgb('GreenYellow');%Middle Color
                    c3=rgb('Lime');%Darkest color
                    c=makeColorMap(c3, c2, c1);
                case {'Blue'}
                    c1=rgb('LightBlue');%brightest color
                    c2=rgb('RoyalBlue');%Middle Color
                    c3=rgb('DarkBlue');%Darkest color
                    c=makeColorMap(c3, c2, c1);
                case {'Purple'}
                    c1=rgb('Thistle');%brightest color
                    c2=rgb('DarkViolet');%Middle Color
                    c3=rgb('Indigo');%Darkest color
                    c=makeColorMap(c3, c2, c1);
                case {'Dirty'}
                    c=makeColorMap([0 0 0], [0.5 0.5 0], [1 1 1]);
                case {'Pink'}
                    c1=rgb('Pink');%brightest color
                    c2=rgb('HotPink');%Middle Color
                    c3=rgb('DeepPink');%Darkest color
                    c=makeColorMap(c3, c2, c1); 
                case {'Gray'}
                    c1=rgb('LightGray');%brightest color
                    c2=rgb('DarkGray');%Middle Color
                    c3=rgb('Black');%Darkest color
                    c=makeColorMap(c3, c2, c1);
                case {'Orange'}
                    c1=rgb('Wheat');%brightest color
                    c2=rgb('DarkOrange');%Middle Color
                    c3=rgb('OrangeRed');%Darkest color
                    c=makeColorMap(c3, c2, c1);
                case {'DarkGreen'}
                    c1=rgb('Aquamarine');%brightest color
                    c2=rgb('MediumSeaGreen');%Middle Color
                    c3=rgb('DarkGreen');%Darkest color
                    c=makeColorMap(c3, c2, c1);    
                case {'Cyan'}
                    c1=rgb('LightCyan');%brightest color
                    c2=rgb('Cyan');%Middle Color
                    c3=rgb('DarkCyan');%Darkest color
                    c=makeColorMap(c3, c2, c1); 
                case {'Brown'}
                    c1=rgb('BurlyWood');%brightest color
                    c2=rgb('Chocolate');%Middle Color
                    c3=rgb('Maroon');%Darkest color
                    c=makeColorMap(c3, c2, c1); 
            end
                    
             % gray to red ([0 0 0], [1 0 0], [1 1 1]), gray to green ([0 0 0], [0 1 0], [1 1 1]), gray to blue ([0 0 0], [0 0 1], [1 1 1]), gray to purple ([0 0 0], [0.5 0 0.5], [1 1 1]),ddong ([0 0 0], [0.5 0.5 0], [1 1 1])
            c=flipud(c);
            colormap (c);
            F = F./max(F(:));
