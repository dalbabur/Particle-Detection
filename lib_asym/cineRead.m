function [cdata] = cineRead(file_dir_img_loc, fileName,frameNum,info,LinLUT)
%
...
% updated 12142011 Henry Tse (unpacks compressed 10bit packed arrays & transforms to 12 bit LUT)
% check inputs
...
%         info=cineInfo(file_dir_img_loc,fileName);
...
%             cd(file_dir_img_loc)
            fileNamePath=[file_dir_img_loc '/' fileName];
            % get a handle to the file from the filename
%             f1=fopen(fileName);
            f1=fopen(fileNamePath);
            ...
            elseif info.biBitCount==16 % 10-, 12-, 14-, or 16-bit gray
                % For packed 10/12-bit files in 8 bit packed arrays
                if info.biCompression==256 
                    idata=fread(f1,ImageSize/1.25,'ubit10=>uint16', 0, 'b');
                    idata=idata+1;
                    idata=LinLUT(idata);
                    clname = 'uint16';
                    nDim=1;
