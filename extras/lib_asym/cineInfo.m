function [info] = cineInfo(file_dir_img_loc,fileName)
%
...
if strcmpi(fileName(end-3:end),'.cin') || ...
  ...
  % use the Phantom SDK libraries if we're on the 32bit windows platform
    ...
    % 64bit Windows, etc.)
      ...
      % cd(file_dir_img_loc)
      fileName = [file_dir_img_loc '/' fileName];
      ...
      % Identifies if cine is saved in packed array form
      info.biCompression = header32(5 + (info.OffImageHeader/4));
